    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                elif active_device and active_window == "TERM":
                    term_input.handle_event(event)
                    if event.key == pygame.K_RETURN:
                        cmd_line = term_input.text.strip('> ').lower()
                        ...
                        if not cmd_line:
                            ...
                            if cmd == "help":
                                term_output.append("HELP - Beschikbare commando's:")
                                term_output.append("  ipconfig : Toont IP instellingen")
                                term_output.append("  ping <ip>: Test verbinding met IP")
                            elif cmd == "ipconfig":
                                term_output.append(f"IPv4 Adres . . . . . : {active_device.ip or '0.0.0.0'}")
                                term_output.append(f"Subnet Mask. . . . . : {active_device.subnet or '0.0.0.0'}")
                                term_output.append(f"Standaardgateway . . : {active_device.gateway or '0.0.0.0'}")
                                if active_device.type in ('PC', 'Laptop'):
                                    term_output.append(f"DNS-servers. . . . . : {active_device.dns or '0.0.0.0'}")
                            elif cmd == "ping":
                                if len(parts) < 2:
                                    term_output.append("Gebruik: ping <ip-adres>")
                                else:
                                    target_ip = parts[1]
                                    term_output.append(f"Pingen naar {target_ip} met 32 bytes aan gegevens:")
                                    found = False
                                    for d in devices:
                                        if d.ip == target_ip or (getattr(d, 'public_ip', None) == target_ip):
                                            found = True
                                            break
                                    if found:
                                        term_output.append(f"Antwoord van {target_ip}: bytes=32 tijd=14ms TTL=128")
                                        m = mission_sys.get_current()
                                ...
                                term_output.append(f"'{cmd}' wordt niet herkend als commando.")
                            
                            term_input.text = ">"
                            while len(term_output) > 12: term_output.pop(0)
                else:
                ...
                if event.button == 1: 
                    ...
                    # OS WINDOW CLICKS
                    if active_device:
                        # Gebruik de juiste box-afmetingen afhankelijk van het venster
                        if active_window == "ISP":
                            ...
                            for key, icon_file in options:
                                ...
                                if r.collidepoint(event.pos):
                                    if key == "ip_settings":
                                        ...
                                        ui_alpha = 0
                                        term_input.text = ">"
                                        term_input.active = True
                                        term_output.clear()
                                        term_output.append(f"{active_device.type} Console v1.0")
                                        term_output.append("Type 'help' for commands.")
                                    elif key == "wifi":
        ...
        if active_device:
            ...
            if active_window == "MENU":
                ...
                box = pygame.Rect(WIDTH//2 - 280, HEIGHT//2 - 200, 560, 400)
                pygame.draw.rect(os_surf, (30, 30, 35), box)
                pygame.draw.rect(os_surf, (20, 20, 25), (box.x, box.y, box.width, 30))
                pygame.draw.rect(os_surf, (52, 50, 199), box, 3)
